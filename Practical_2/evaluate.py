...
class BayesianEvaluator:
    def __init__(self, w2i, window_size):
        self.w2i = w2i
        self.window_size = window_size
        self.sentences = self.load_data()
    ...
    def load_data(self):
        ...
        for sentence in data:
            ...
            # Fix size
            pre = pre[-min(len(pre), self.window_size):]
            post = post[:min(len(post), self.window_size)]

            pre = (self.window_size-len(pre))*['<unk>'] + pre
            post = post + (self.window_size-len(post))*['<unk>']
            context = pre + post
    ...
    def lst(self, encoder, priorMu, priorSigma):
        ...
        for word_id, sentence_id, word, context, candidates in self.sentences:
            # Compute scores
            ranking = []
            mu_posterior, sigma_posterior = encoder(torch.LongTensor([[self.w2i[word]]]).cuda(), torch.LongTensor([[self.w2i[c] for c in context]]).cuda())
            for candidate in candidates:
                mu_prior = priorMu(torch.LongTensor([self.w2i[candidate]]).cuda())
                sigma_prior = priorSigma(torch.LongTensor([self.w2i[candidate]]).cuda())
